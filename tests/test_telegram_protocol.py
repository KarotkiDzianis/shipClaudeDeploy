...
class TestApprovalStore(unittest.TestCase):
    ...
    def test_valid_approval_succeeds(self):
        ...
        result = self.store.decide("demo", "r1", approval.nonce, 111, "approved")
        self.assertEqual(result, {"status": "ok", "decision": "approved", "idempotent_repeat": False})

    def test_wrong_user_rejected(self):
        approval = self.store.create_pending("demo", "me/demo", "sha1", "r1")
        result = self.store.decide("demo", "r1", approval.nonce, 999, "approved")
        self.assertEqual(result["status"], "rejected")

    def test_duplicate_click_is_idempotent_not_a_re_trigger(self):
        approval = self.store.create_pending("demo", "me/demo", "sha1", "r1")
        first = self.store.decide("demo", "r1", approval.nonce, 111, "approved")
        second = self.store.decide("demo", "r1", approval.nonce, 111, "approved")
    ...
    def test_duplicate_click_by_a_different_allowed_user_still_returns_original_decision(self):
        approval = self.store.create_pending("demo", "me/demo", "sha1", "r1")
        self.store.decide("demo", "r1", approval.nonce, 111, "approved")
        race = self.store.decide("demo", "r1", approval.nonce, 222, "blocked")
    ...
    def test_old_release_callback_rejected_after_new_one_supersedes_it(self):
        ...
        result = self.store.decide("demo", "r1", first.nonce, 111, "approved")
        self.assertEqual(result["status"], "rejected")

    def test_stale_nonce_for_current_release_id_rejected(self):
        approval = self.store.create_pending("demo", "me/demo", "sha1", "r1")
        result = self.store.decide("demo", "r1", "totally-wrong-nonce", 111, "approved")
        self.assertEqual(result["status"], "rejected")

    def test_unknown_decision_value_rejected(self):
        approval = self.store.create_pending("demo", "me/demo", "sha1", "r1")
        result = self.store.decide("demo", "r1", approval.nonce, 111, "maybe")
        self.assertEqual(result["status"], "rejected")

    def test_blocked_release_cannot_later_approve_without_a_new_approval(self):
        approval = self.store.create_pending("demo", "me/demo", "sha1", "r1")
        self.store.decide("demo", "r1", approval.nonce, 111, "blocked")
        # clicking "approved" on the SAME (already blocked) release must
        # NOT flip it -- it's idempotent to the recorded "blocked" decision
        retry = self.store.decide("demo", "r1", approval.nonce, 111, "approved")
        ...
        fresh = self.store.decide("demo", "r2", new_approval.nonce, 111, "approved")
    ...
    def test_decide_on_project_with_no_pending_approval_rejected(self):
        result = self.store.decide("never-created", "r1", "any-nonce", 111, "approved")
        self.assertEqual(result["status"], "rejected")
